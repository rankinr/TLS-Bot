...
def processPost(post_content_page):
	...
	for urmv in urms:
		for matchval in urms[urmv]:
			if post_content.lower().count(' '+matchval.lower()+' ') != 0 or post_content.lower().count(' '+matchval.lower()+'.') != 0:
				urm=urmv
	return {'lsat':highest_lsat,'gpa':highest_gpa,'urm':urm,'posts':num_posts}

def generate_reply(lsat,gpa,urm,num_posts):
	reply_text=[]
	if num_posts < 10: reply_text.append('Welcome to TLS!')
	if highest_gpa == 10 or highest_lsat == 0: reply_text.append("It doesn't look like you've told us your LSAT and GPA. Many factors might affect your law school applications, but most TLS users agree that LSAT and GPA are usually the two most important. Please edit your post to include your LSAT and LSAC GPA. If you have not yet taken the LSAT, a range of practice test scores is okay, too.")
	else:
		image_link=getImageLink(highest_lsat,highest_gpa,urm)
		# ONCE CAN POST IMAGES: reply_text.append("Since you've provided your LSAT ("+str(highest_lsat)+") and GPA ("+str(highest_gpa)+"), I have generated an image with an overview of relevant admissions data on [url=http://lawschoolnumbers.com]LawSchoolNumbers.com[/url].\n\nHere is your image: http://mylsn.info/"+image_link+"_1-14.jpg")
		reply_text.append("Since you've provided your LSAT ("+str(highest_lsat)+") and GPA ("+str(highest_gpa)+"), I have generated a graph with an overview of relevant admissions data on [url=http://lawschoolnumbers.com]LawSchoolNumbers.com[/url].\n\nHere is your image: http://mylsn.info/"+image_link)
		...
		for post in posts:
			if post.getText().strip() != 'Topics': # first reference is headings, ignore this line
				...
				if int(tid) > int(start_most_recent_check): # was this thread posted after the most recent one we checked?
					...
					reply_text=generate_reply(post_data['lsat'],post_data['gpa'],post_data['urm'],post_data['posts']) # GENERATE REPLY TEXT
